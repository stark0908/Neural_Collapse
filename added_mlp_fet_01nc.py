...
def train_model(train_domains, test_domains, frac, cfg):
    ...
    # FIX 4: Use max so larger domains aren't truncated.
    # Shorter loaders will be wrapped with cycle() selectively below.
    ...
    for epoch in pbar:
        # FIX 1: Recreate iterators fresh each epoch so shuffle=True takes effect.
        # FIX 4 (continued): Only wrap shorter loaders in cycle(); the longest runs fully.
        train_iters = [
        ...
        for _ in range(steps_per_epoch):
            ...
            feats_list  = []
            labels_list = []  # FIX 5: collect labels to compute NC loss jointly
            ...
            for x, y in batches:
                ...
                labels_list.append(y)   # store per-domain labels

            # FIX 5: NC loss computed ONCE over ALL domains concatenated,
            # not once per domain inside the loop (which compounded the loss
            # and ignored cross-domain class structure).
            if cfg["nc"] and epoch >= nc_start_epoch:
                all_feats  = torch.cat(feats_list,  dim=0)
                all_labels = torch.cat(labels_list, dim=0)
                total_loss += lambda_nc * nc_loss(all_feats, all_labels)

            # FIX 2: Guard CORAL against silent skip when any batch is too small.
            # Original code had no warning — CORAL could silently contribute 0.
        ...
        avg_train_loss = epoch_train_loss / max(steps_per_epoch, 1)
    ...
    for train_domains, test_domains in regimes:
        for frac in fractions:
            for cfg in methods:
                ...
                avg_acc, worst_acc, train_loss, val_loss = train_model(
                    train_domains,
                    test_domains,
                    frac,
                    cfg
                ...
                # atomic save — avoids CSV corruption on interrupt
