...
def train_model(train_domains, test_domains, frac, cfg):
    ...
    for epoch in pbar:
        train_iters = [cycle(loader) for loader in train_loaders]  # recreate each epoch for reshuffling
        model.train()
    ...
    print(f"Avg Acc: {avg_acc:.4f}, Worst: {worst_acc:.4f}, Val Loss: {avg_val_loss:.4f}, Train Loss: {avg_train_loss:.4f}")
