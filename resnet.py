...
class CLIPModel(nn.Module):
    def __init__(self, num_classes):
        ...
        # 🔁 Replace CLIP ViT with ResNet-50
        resnet = models.resnet50(pretrained=True)

        # remove classification head
        self.encoder = nn.Sequential(*list(resnet.children())[:-1])  # [B, 2048, 1, 1]

        # project to 512 (to match CLIP dim)
        self.proj = nn.Linear(2048, 512)

        # freeze encoder (same as CLIP freeze)
        for p in self.encoder.parameters():
            p.requires_grad = False

        # keep your head EXACTLY same
    ...
    def forward(self, x, return_feats=False):
        ...
        feats_raw  = feats                      # pre-norm (CORAL)
        feats_norm = F.normalize(feats, dim=1)  # post-norm (CE + NC)
